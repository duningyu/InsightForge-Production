    ...
    for (const viewport of VIEWPORTS) {
      ...
      await page.evaluate(() => {
        ...
        state.solutions = {selected_candidate_id: 'solution-a', candidates: [
          {id: 'solution-a', title: '分步求职准备', mechanism: 'rule_based', summary: '先收集目标，再按规则给出下一步。', why_fit: '帮助初学者逐步整理求职材料。', complexity: 'low', data_requirements: ['用户输入'], risks: ['用户需求仍需确认'], user_flow: ['输入目标', '获得建议'], mvp_pages: ['项目首页'], features: ['逐步引导'], inputs: ['想法'], outputs: ['行动清单'], decision_logic: ['按用户目标收敛'], technical_components: ['Web 应用'], implementation_plan: ['先完成输入与反馈'], acceptance_cases: ['可完成一次流程'], unknowns: ['目标用户需要访谈确认'], provenance: 'model_hypothesis', required_data_class: 'synthetic', automation_level: 'low', human_role: 'reviews', core_decision_logic: '按用户目标收敛', major_dependency: 'none', requires_llm_runtime: false, requires_rag_runtime: false, requires_agent_runtime: false},
          {id: 'solution-b', title: '资料整理工作台', mechanism: 'workflow_based', summary: '把资料集中后由用户逐项整理和确认。', why_fit: '把已有资料集中整理。', complexity: 'medium', data_requirements: ['用户资料'], risks: ['资料质量待确认'], user_flow: ['上传资料', '逐项整理', '确认结果'], mvp_pages: ['资料页'], features: ['资料整理'], inputs: ['资料'], outputs: ['整理结果'], decision_logic: ['按资料类型组织'], technical_components: ['存储'], implementation_plan: ['先完成资料入口'], acceptance_cases: ['资料可查看'], unknowns: ['资料来源待确认'], provenance: 'model_hypothesis', required_data_class: 'synthetic', automation_level: 'low', human_role: 'synthetic author', core_decision_logic: '按资料类型组织', major_dependency: 'none', requires_llm_runtime: false, requires_rag_runtime: false, requires_agent_runtime: false},
          {id: 'solution-c', title: '轻量反馈循环', mechanism: 'prediction_based', summary: '根据历史进度估计下一步风险，再由用户验证。', why_fit: '通过小步反馈验证方向。', complexity: 'medium', data_requirements: ['历史反馈'], risks: ['预测需要更多样本'], user_flow: ['输入历史', '查看风险', '验证建议'], mvp_pages: ['反馈页'], features: ['反馈记录'], inputs: ['反馈'], outputs: ['待办事项'], decision_logic: ['按反馈排序'], technical_components: ['表单'], implementation_plan: ['先完成反馈记录'], acceptance_cases: ['反馈可保存'], unknowns: ['反馈频率待确认'], provenance: 'model_hypothesis', required_data_class: 'synthetic', automation_level: 'medium', human_role: 'validates forecast', core_decision_logic: '按反馈排序', major_dependency: '历史反馈', requires_llm_runtime: false, requires_rag_runtime: false, requires_agent_runtime: false},
        ]};
      ...
      const metrics = await page.evaluate(() => {
        ...
        const cols = solutionGrid ? getComputedStyle(solutionGrid).gridTemplateColumns.split(' ').length : 0;
