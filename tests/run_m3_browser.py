"""Actual loopback HTTP/browser M3 journey; temporary data, no real secrets."""
import json
import os
...
with tempfile.TemporaryDirectory(prefix="insightforge-m3-browser-") as temporary:
    temporary_path = Path(temporary)
    os.environ["INSIGHTFORGE_DATABASE_PATH"] = str(temporary_path / "unused.sqlite3")
    os.environ["RUNTIME_DIR"] = str(temporary_path / "unused-runtime")
    ...
    try:
        ...
        result = subprocess.run(
            ["node", str(ROOT / "tests" / "m3_flow_browser.cjs")],
            input=json.dumps(data),
            text=True,
            cwd=ROOT,
        )
        assert result.returncode == 0, "M3 browser scenario failed"
        assert not blocked, "External attempt detected"
        print("M3 browser journey completed; real external connections=0; blocked attempts=0")
    finally:
