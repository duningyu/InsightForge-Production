// Candidate UI only. This is NOT the AI/decision/document vertical slice proof.
...
(async () => {
  ...
  try {
    ...
    await page.locator('[name=username]').fill('competitor-browser');
    ...
    page.once('dialog',d=>d.accept('Synthetic competitor UI'));
    ...
    console.log('PASS Chromium candidate-only: UI create two/view/select one/Esc focus/reopen/remove/skip; Provider/Search attempts=0. AI/snapshot/doc flow NOT_RUN.');
  } catch(error) {
