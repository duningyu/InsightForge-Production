...
def install_fake_transport(counter):
    ...
    def payload(request):
        ...
        if "conservative brainstorming" in system.lower() or "reference suggestions" in system.lower():
...
with tempfile.TemporaryDirectory(prefix="insightforge-ai-browser-") as td:
    ...
    counter=td/"counts.json"; counter.write_text(json.dumps({"ai_reference":0,"generation":0}))
    ...
    try:
        ...
        counts=json.loads(counter.read_text()); assert result.returncode==0, "browser scenario failed"; assert counts["ai_reference"]==1, counts; assert not blocked,blocked
